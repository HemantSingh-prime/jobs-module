...
public class JobControllerTest {
	...
	void initUseCase() throws Exception {
		...
		secondrySkill = objectMapper.readValue(new File("src/test/java/com/ps/js/payload/secondry-skill.json"),
				Skill.class);
		/*rolesAndResponsebility = objectMapper.readValue(
				new File("src/test/java/com/ps/js/payload/roles-and-responsebility.json"),
				RolesAndResponsebility.class);
		rolesAndResponsebility2 = objectMapper.readValue(
				new File("src/test/java/com/ps/js/payload/roles-and-responsebility2.json"),
				RolesAndResponsebility.class);*/
	...
	void openJobs_successful() throws Exception {
		...
		/*Mockito.when(rolesAndResponsebilityServiceImpl.fetchRolesAndResponsebilityById(1))
				.thenReturn(optionalRolesAndResponsebility);
		Mockito.when(rolesAndResponsebilityServiceImpl.fetchRolesAndResponsebilityById(2))
	...
	void findAllJobLocation_successful() throws Exception {
		List<JobLocation> listLocation = new ArrayList();
...
//	}
	...
	void findAllJob_successful()throws Exception{
		...
		mockMvc.perform(get("/jobs/find-all-job/").contentType(MediaType.APPLICATION_JSON)
				.characterEncoding("utf-8").accept(MediaType.APPLICATION_JSON)).andExpect(status().is2xxSuccessful());
	}
